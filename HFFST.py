...
class HFFST(pl.LightningModule):
    ...
    def load_backbone(self, backbone, weights_path):
        if backbone in ["resnet34", "resnet18"]:
            model = load_model_weights(backbone, weights_path)
    ...
    def forward(self, x, x_total, position, neighbor, mask, pid=None, sid=None):
        x = x.cuda()
        mask = mask.cuda()
        if type(x_total) == list:
            x_total = [i.squeeze() for i in x_total]
        else:
            x_total = x_total.squeeze()
        
        # Target tokens
        fine_token = self.fine_tokenizer(x)

        if self.is_swintransformer == True:
            B, N, C = fine_token.shape
            H = W = int(N ** 0.5)
        ...
        fine_token = self.fine_encoder(fine_token)
        ...
        # Coarse tokens
        coarse_token = self.coarse_encoder(neighbor.cuda(), mask)

        # fusion tokens
        fusion1_token = self.fusion1(medium_token.unsqueeze(1), coarse_token)
        fusion2_token = self.fusion2(medium_token.unsqueeze(1), fine_token)
        fusion3_token = self.fusion3(fusion1_token, fusion2_token)

        output_coarse = self.fc_c(coarse_token.mean(1))
        output_medium = self.fc_m(fusion1_token.squeeze())
        output = self.fc_f(fusion3_token.squeeze())
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss += F.mse_loss(output_medium.view_as(exp), exp)*self.w_1
        loss += F.mse_loss(output_coarse.view_as(exp), exp)*self.w_2
    ...
    def on_validation_epoch_end(self):
        ...
        avg_corr = torch.stack([x["corr"] for x in outputs])
    ...
    def test_step(self, batch, batch_idx):
        ...
        if '10x_breast' in name[0]:
            ...
            ind_match = np.load(rf'{name[0]}\ind_match.npy', allow_pickle=True)
        ...
        os.makedirs(f"result/{self.__class__.__name__}/{self.data}/{self.patient}", exist_ok=True)
        np.save(f"result/{self.__class__.__name__}/{self.data}/{self.patient}/{name[0]}", pred.T)
        output = {"MSE":mse, "MAE":mae, "corr":rr}
    ...
    def on_test_epoch_end(self):
        ...
        os.makedirs(f"result/{self.__class__.__name__}/{self.data}/{self.patient}", exist_ok=True)
        torch.save(avg_mse.cpu(), f"result/{self.__class__.__name__}/{self.data}/{self.patient}/MSE")
        torch.save(avg_mae.cpu(), f"result/{self.__class__.__name__}/{self.data}/{self.patient}/MAE")
        torch.save(avg_corr.cpu(), f"result/{self.__class__.__name__}/{self.data}/{self.patient}/cor")
